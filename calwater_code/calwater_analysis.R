...
### Crisis
calwater_mod2 = calwater_mod %>% filter(short_clean2 %in% c(2))
...
plot(model2, pch=16, col="black")
mtext("OLS Crisis Model Diagnostics", line = 0.5, outer = TRUE, cex = 1.5)

dev.print(png, file=paste(op_path, "diag_olsmod_crisis.png", sep="/"), width=870, height=615, units="px")

#calwater_mod %>% filter(is.na(share_resid))

table(calwater$hydrologic_region, calwater$owner)
